...
from .Items import item_name_to_id, item_id_to_name, item_table, is_joker, is_joker_bundle, jokers, decks, joker_bundles, offset, ItemData, QualatroItem, \
...
from .QualatroDecks import deck_id_to_name, deck_name_to_key
import math
from worlds.generic.Rules import add_rule
from .Options import QualatroOptions, Traps, IncludeDecksMode, StakeUnlockMode, \
    IncludeStakesMode, Goal, TarotBundle, SpectralBundle, PlanetBundle
from Options import OptionError
from .Locations import QualatroLocation, qualatro_location_id_to_name, qualatro_location_name_to_id, \
    qualatro_location_id_to_stake, shop_id_offset, qualatro_location_id_to_ante, max_shop_items, consumable_id_offset


class QualatroWebWorld(WebWorld):
    theme = "partyTime"
    bug_report_page = "https://discord.gg/dESF3rzxQu"
    rich_text_options_doc = True
    setup_en = Tutorial(
        "Multiworld Setup Guide",
        "A guide to setting up Qualatro on your computer.",
        "English",
        "setup_en.md",
        "setup/en",
        ["Burndi"]
...
class QualatroWorld(World):
    ...
    """
    game = "Qualatro"
    web = QualatroWebWorld()

    topology_present = False

    options_dataclass = QualatroOptions
    options: QualatroOptions
    ...
    location_id_to_name = qualatro_location_id_to_name
    location_name_to_id = qualatro_location_name_to_id
    ...
    def create_item(self, item: Union[str, ItemData], classification: ItemClassification = None) -> QualatroItem:
        item_name = ""
        ...
        return QualatroItem(item_name, classification, item.code, self.player)

    def create_regions(self) -> None:
        menu_region = Region("Menu", self.player, self.multiworld)

        self.multiworld.regions.append(menu_region)
        all_locations: List[QualatroLocation] = list()
        ...
        for deck in deck_id_to_name:
            ...
            if (deck_name in self.playable_decks):
                ...
                for location in qualatro_location_name_to_id:
                    if str(location).startswith(deck_name):
                        location_id = qualatro_location_name_to_id[location]
                        stake = qualatro_location_id_to_stake[location_id]
                        ante = qualatro_location_id_to_ante[location_id]

                        new_location = QualatroLocation(
        ...
        def can_reach_count(state: CollectionState, locations: List[QualatroLocation], count: int = 1) -> bool:
            counter = 0
        ...
        for location in qualatro_location_name_to_id:
            if str(location).startswith("Shop Item"):
                self.shop_locations[qualatro_location_name_to_id[location]] = location
        ...
        for location in qualatro_location_name_to_id:
            if str(location).startswith("Consumable Item") and counter < self.options.ap_consumable_items.value:
                counter += 1
                self.consumable_locations[qualatro_location_name_to_id[location]] = location
